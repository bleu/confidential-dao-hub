  ...
  return (
    <div className="pb-24">
      ...
      <section className="mt-6 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 sm:p-7">
        ...
        <div className="mt-6 overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950/60">
          <div className="hidden border-b border-zinc-800 px-3 py-2 font-mono text-xs text-zinc-500 md:grid md:grid-cols-[minmax(0,1fr)_10rem_auto] md:gap-3">
            <span>Payments</span>
            <span>Amounts</span>
            <span className="sr-only">Actions</span>
          </div>
          <div className="divide-y divide-zinc-800">{entries.map((entry) => <PaymentRow key={entry.id} entry={entry} validation={validation} token={token} showValidation={showValidation} touched={touched[entry.id]} locked={sending} canRemove={entries.length > 1} onChange={updateEntry} onBlur={(id, field) => setTouched((current) => ({ ...current, [id]: { ...current[id], [field]: true } }))} onRemove={(id) => setEntries((current) => current.length > 1 ? current.filter((entry) => entry.id !== id) : current)} />)}</div>
        </div>
...
function PaymentRow({ entry, validation, token, showValidation, touched, locked, canRemove, onChange, onBlur, onRemove }: { entry: PaymentEntry; validation: Validation; token?: Token; showValidation: boolean; touched?: TouchedFields[string]; locked: boolean; canRemove: boolean; onChange: (id: string, field: "recipient" | "amount", value: string) => void; onBlur: (id: string, field: "recipient" | "amount") => void; onRemove: (id: string) => void }) {
