...
export function VariantA({ area, page, go, children }: LayoutProps) {
  return <div className="grid overflow-hidden rounded-xl border border-zinc-800 md:grid-cols-[185px_1fr]">
    <aside className="border-b border-zinc-800 bg-zinc-900/50 p-4 md:border-r md:border-b-0">
      ...
      <p className="mb-3 text-[10px] uppercase tracking-widest text-zinc-500">Workspace</p>
      ...
      <p className="mt-10 text-xs leading-5 text-zinc-500">Public pages.<br />Private amounts.</p>
    </aside>
...
export default function WorkspacePrototype() {
  ...
  const intro = (eyebrow: string, title: string, text: string) => <div className="mb-7"><p className="mb-3 font-mono text-[10px] uppercase tracking-[0.2em] text-yellow-300">{eyebrow}</p><h1 className="text-3xl font-medium tracking-tight text-zinc-100">{title}</h1><p className="mt-3 text-sm leading-6 text-zinc-400">{text}</p></div>;
  ...
  if (area === "start") {
    const choices = [{ area: "community" as const, number: "01", title: "Community", text: "Find a buyback. Sell your tokens. Follow what is owed to your wallet.", action: "Explore buybacks" }, { area: "dao" as const, number: "02", title: "DAO dashboard", text: "Inspect your DAO's operations. Use treasury tools with an authorized wallet.", action: "View Demo DAO" }];
    content = <>{intro("Confidential Ops Hub", "Where would you like to go?", "Both areas are open to everyone. Your wallet controls which private amounts and actions are available.")}
      <div className={variant === "B" ? "grid gap-4 sm:grid-cols-2" : "grid gap-4"}>{choices.map(choice => <button key={choice.area} onClick={() => go(choice.area)} className={variant === "C" ? "group flex items-start gap-5 border-t border-zinc-700 py-6 text-left" : `${panel} group text-left transition hover:border-yellow-500`}><span className="font-mono text-xs text-yellow-400">{choice.number}</span><div><h2 className="mt-3 text-xl text-zinc-100">{choice.title}</h2><p className="mt-3 text-sm leading-6 text-zinc-400">{choice.text}</p><p className="mt-6 text-sm text-yellow-300">{choice.action} &gt;</p></div></button>)}</div><p className="mt-5 text-xs text-zinc-500">One DAO on Sepolia. No wallet needed to browse.</p></>;
  } else if (page === "report") {
    content = <>{intro("Demo DAO / Buybacks", "Public buyback report", "The same report is available to community members and treasury teams.")}<div className={panel}><h2 className="text-lg text-zinc-100">Epoch 12</h2><p className="mt-2 text-sm text-zinc-400">Sample closed epoch. Aggregate totals have not been disclosed.</p><dl className="mt-5 divide-y divide-zinc-800 text-sm">{["Total sold", "Total paid"].map(label => <div key={label} className="flex flex-wrap justify-between gap-2 py-4"><dt className="text-zinc-400">{label}</dt><dd>Not disclosed</dd></div>)}</dl><p className="mt-4 text-xs leading-5 text-zinc-500">Only eligible aggregate totals can become public. Individual offers and price floors stay private. This prototype does not request disclosure.</p></div><button className={`${button} mt-5`} onClick={() => go(area, area === "dao" ? "buybacks" : "detail")}>Back to buyback</button></>;
  } else if (page === "vesting" || page === "payroll") {
    content = <>{intro(area === "community" ? "Community / Your wallet" : "Demo DAO / Operations", area === "community" ? `My ${page}` : labels[page], area === "community" ? (page === "vesting" ? "Your grants will appear here, including grants from different DAOs." : "Payments to your wallet will appear here, including payments from different DAOs.") : (page === "vesting" ? "Create and manage grants from this DAO." : "Send confidential payments from this DAO."))}<div className={`${panel} py-12 text-center`}><span className="rounded border border-zinc-700 px-3 py-1 text-xs text-zinc-400">Soon</span><p className="mt-5 text-sm text-zinc-400">This feature has no app page yet.</p><p className="mt-2 text-xs text-zinc-500">No wallet data is loaded in this preview.</p></div></>;
  } else if (area === "dao" && page === "overview") {
    content = <>{intro("DAO dashboard / Sepolia", "Demo DAO", "View public operations here. Connect the treasury wallet to manage buybacks.")}<p className="mb-6 text-xs text-zinc-500">Demo DAO is a prototype name for the current cTOKEN deployment.</p><div className="divide-y divide-zinc-800 border-y border-zinc-800">{(["buybacks", "vesting", "payroll"] as const).map(item => <button key={item} onClick={() => go("dao", item)} className="flex w-full items-center justify-between gap-4 py-6 text-left hover:text-yellow-300"><div><h2 className="text-lg">{labels[item]}</h2><p className="mt-1 text-xs text-zinc-500">{item === "buybacks" ? "cTOKEN / Treasury tools and public reports" : "Confidential treasury operations"}</p></div><span className="text-xs text-yellow-300">{item === "buybacks" ? "Open >" : "Soon"}</span></button>)}</div></>;
  } else if (area === "community" && page === "buybacks") {
    content = <>{intro("Community / Buybacks", "Find a buyback", "Choose a token to sell back to its DAO. Each buyback contains its own settlement epochs.")}<div className={variant === "C" ? "border-y border-zinc-700 py-6" : panel}><div className="flex flex-wrap items-center justify-between gap-3"><span className="font-mono text-xs text-zinc-400">Demo DAO / Sepolia</span><span className="rounded border border-yellow-800 px-2 py-1 text-xs text-yellow-300">Sample open epoch</span></div><h2 className="mt-5 text-2xl text-zinc-100">cTOKEN buyback</h2><p className="mt-2 text-sm text-zinc-400">Sell cTOKEN for cUSDT with a private price floor.</p><div className="mt-6 flex flex-wrap items-center gap-4"><button className={primary} onClick={() => go("community", "detail")}>Open buyback</button>{reportLink}</div></div><p className="mt-5 text-xs text-zinc-500">Showing the one configured buyback. More DAOs can be listed here later.</p></>;
  ...
  return <div className="pb-24">
    <div className="mb-6 rounded-lg border border-dashed border-yellow-800 bg-yellow-950/10 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-mono text-xs text-yellow-300">THROWAWAY UI PROTOTYPE</p><p className="mt-1 text-xs text-zinc-400">Sample data. No signatures or transactions. The header wallet is not used.</p></div><label className="text-xs text-zinc-400">Simulate wallet<select aria-label="Simulate wallet" value={wallet} onChange={event => { setWallet(event.target.value as Wallet); setDecrypted(false); setNotice(""); }} className="ml-2 rounded border border-zinc-700 bg-zinc-950 p-2 text-zinc-100"><option value="visitor">Disconnected</option><option value="holder">Community wallet</option><option value="treasury">Treasury wallet</option></select></label></div></div>
