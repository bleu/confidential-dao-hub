...
| [Vesting](docs/features/vesting.md) | Live discovery and private inspection | Actions pending | [ADR-0006](docs/adr/0006-vesting.md) |
...
Payroll has a mock frontend demo at `/dao/payroll` and `/community/payroll`. The DAO screen simulates a sender flow, and the Community screen shows only the demo receiver's own payments after mock signing. It has no wallet, RPC, signature, transaction, or saved salary data. The caller-funded confidential multisend contract, local tests, and source-verified Sepolia deployment remain separate from this demo; live payment checks and frontend integration are pending. Vesting reads the connected Sepolia wallet, discovers matching grants, and lets the fixed treasury or recipient decrypt grant amounts. It uses the deployed shared confidential grant contract. Creation, claim, revocation, and refund actions remain pending. The other future operations have Soon pages in both workspaces, and their ADRs remain proposed.
...
| `/dao/vesting` | Created vesting grant records |
...
Vesting configuration lives in `frontend/src/features/vesting/contracts.ts`. It contains the deployed Sepolia contract address, discovery start block, ABI, and a decryption scope limited to that contract. See the [vesting guide](docs/features/vesting.md) for the live discovery and private-inspection flow.
...
Buybacks, payroll, and vesting use shared guarded deployment helpers. Their feature-specific commands, environment variables, and operational limits are in the [deployment guide](docs/deployment.md). Payroll is deployed on Sepolia, but its available frontend is a mock demo only; live payment integration remains pending. Vesting is deployed on Sepolia and its frontend discovers and privately inspects grants, but its transaction actions remain pending.
