...
describe("ConfidentialMultisend", function () {
  ...
  it("reports actual outcomes when unsupported custom transfer restrictions split a batch", async function () {
    const asset = await (await ethers.getContractFactory("PayrollTestToken")).deploy();
    await asset.setOperator(scope, (await time.latest()) + 3_600);
    await asset.limitTransfer(3, 0);
    const input = await encrypt([25n, 40n]);
    await multisend.multisend(await asset.getAddress(), [alice.address, bob.address], input.handles, input.inputProof);
    expect(await balance(sender, asset)).to.equal(935n);
    expect(await balance(alice, asset)).to.equal(25n);
    expect(await balance(bob, asset)).to.equal(0n);
    const logs = await multisend.queryFilter(multisend.filters.Payment());
    expect(await fhevm.userDecryptEuint(FhevmType.euint64, logs[0].args.actualAmount, scope, sender)).to.equal(25n);
    expect(await fhevm.userDecryptEuint(FhevmType.euint64, logs[1].args.actualAmount, scope, sender)).to.equal(0n);
  });

  it("leaves a custom token's partial pull unused instead of treating it as full funding", async function () {
    const asset = await (await ethers.getContractFactory("PayrollTestToken")).deploy();
    await asset.setOperator(scope, (await time.latest()) + 3_600);
    await asset.limitTransfer(1, 1);
    const input = await encrypt([25n, 40n]);
    await multisend.multisend(await asset.getAddress(), [alice.address, bob.address], input.handles, input.inputProof);
    expect(await balance(sender, asset)).to.equal(999n);
    expect(await balance(alice, asset)).to.equal(0n);
    expect(await balance(bob, asset)).to.equal(0n);
  });

  it("rolls back the pull, earlier payments, and logs when any token transfer reverts", async function () {
    const asset = await (await ethers.getContractFactory("PayrollTestToken")).deploy();
    await asset.setOperator(scope, (await time.latest()) + 3_600);
    const input = await encrypt([25n, 40n]);
    for (const transfer of [1, 3]) {
      await asset.revertOnTransfer(transfer);
      const block = await ethers.provider.getBlockNumber();
      await expect(
        multisend.multisend(await asset.getAddress(), [alice.address, bob.address], input.handles, input.inputProof, {
          gasLimit: 12_000_000,
        }),
      )
        .to.be.revertedWithCustomError(asset, "TransferRejected")
        .withArgs(transfer);
      expect(await balance(sender, asset)).to.equal(1_000n);
      expect(await balance(alice, asset)).to.equal(0n);
      expect(await balance(bob, asset)).to.equal(0n);
      expect(await multisend.queryFilter(multisend.filters.Payment(), block + 1)).to.have.length(0);
      expect(await asset.queryFilter(asset.filters.ConfidentialTransfer(), block + 1)).to.have.length(0);
    }
  });

  it("isolates actual payment references when a token reuses a zero result", async function () {
    const asset = await (await ethers.getContractFactory("PayrollTestToken")).deploy();
    await asset.setOperator(scope, (await time.latest()) + 3_600);
    await asset.reuseZeroResult();
    const input = await encrypt([0n, 0n]);
    await multisend.multisend(await asset.getAddress(), [alice.address, bob.address], input.handles, input.inputProof);
    const logs = await multisend.queryFilter(multisend.filters.Payment());
    expect(logs[0].args.actualAmount).not.to.equal(logs[1].args.actualAmount);
    expect(await fhevm.userDecryptEuint(FhevmType.euint64, logs[0].args.actualAmount, scope, alice)).to.equal(0n);
    await expect(fhevm.userDecryptEuint(FhevmType.euint64, logs[0].args.actualAmount, scope, bob)).to.be.rejectedWith(
      "not authorized",
    );
  });

  it("blocks token reentry during the pull and later outgoing transfers", async function () {
    const reentrant = await (await ethers.getContractFactory("PayrollTestToken")).deploy();
    await reentrant.setOperator(scope, (await time.latest()) + 3_600);
    const input = await encrypt([25n, 40n]);
    for (const transfer of [1, 3]) {
      await reentrant.reenterOnTransfer(scope, transfer);
      await expect(
        multisend.multisend(
      ...
      ).to.be.revertedWithCustomError(multisend, "ReentrancyGuardReentrantCall");
    }
  });
