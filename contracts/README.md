...
- `deploy/buybacks.ts`: the existing buyback deployment names, tag, and identifier.
- `deploy/payroll.ts` and `deploy/vesting.ts`: thin guarded feature deployment wrappers.
- `scripts/deployment/`: shared preflight, deployment, provenance, export, and error helpers.
- `scripts/payroll/` and `scripts/vesting/`: feature configuration, thin command wrappers, and feature-only scripts.

The [deployment guide](../docs/deployment.md) defines the shared payroll and vesting procedure. The generic `deploy:sepolia` and `deploy:localhost` npm commands remain tagged for buybacks only. Use `npm ci`, `npm run compile`, and `npm test` from this directory. Use `npm test -- --grep "ConfidentialMultisend"` for payroll tests only.
