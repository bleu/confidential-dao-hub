...
Zero amounts and repeated recipients are valid. Each transaction is a new payment request, with no saved run ID, expiry rule, duplicate-request protection, or retry accounting. Operator permission expiry belongs to the token and is not an amount-limited allowance. The frontend requires input review and blocks duplicate clicks. It provides a wallet-backed Sepolia proof of concept, but no live payment or private-read check has passed. Follow the delivery stages in [ADR 0009](0009-feature-delivery-stages.md) before making payroll live. Contract behavior and token requirements are documented in the [payroll guide](../features/payroll.md).
